...
class CustomActiveLearning:
    ...
    def run_active_learning(self, features_df, feature_cols, label_col, n_iter=20, max_early_stop=2, early_stop_th=0.1,
        ...
        for idx in range(n_iter):
            ...
            self.data_size.append(X_train.shape[0])
            ...
            # store info
            self.cross_validation_scores.append(iter_res["model"].cross_validation_scores)
            self.test_scores_exp.append(iter_res["model"].test_scores_exp)
            self.cross_validation_scores_exp.append(iter_res["model"].cross_validation_scores_exp)
            self.IQRs_mean.append(np.mean(np.abs(iter_res["uncertainty_interval"])))
            iter_res["model"] = str(iter_res["model"])
            self.iterations_results.append(iter_res)
        ...
        pred_jobs[f"pred_{label_col}"] = self.iterations_results[-1]["pred_labels"]
        pred_jobs[f"unc_low_{label_col}"] = self.iterations_results[-1]["uncertainty_low"]
        pred_jobs[f"unc_up_{label_col}"] = self.iterations_results[-1]["uncertainty_high"]
    ...
    # def active_learning_iteration_helper(self, user_prompt=None, idx, data_size, X_train, y_train, X_test, ids_train, ids_test, feature_cols, verbose=False, cross_validation_scores=None,
    #                                      test_scores_exp=None, cross_validation_scores_exp=None,
    #                                      iterations_results=None):
    #     if (user_prompt):
    #         val = input("Enter x to stop AL process, press any other key to continue: ")
    #         if (val.strip() == "x"):
    #             return
